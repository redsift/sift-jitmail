 ...
 */
var JITMailController = (function (SiftController) {
  function JITMailController() {
  ...
  if ( SiftController ) JITMailController.__proto__ = SiftController;
  JITMailController.prototype = Object.create( SiftController && SiftController.prototype );
  JITMailController.prototype.constructor = JITMailController;

  // TODO: link to docs
  JITMailController.prototype.loadView = function loadView (state) {
    var this$1 = this;

    var ret = { html: state.type + '.html' };
    ret.data = new Promise(function (resolve, reject) {
      this$1.storage.get({ bucket: '_redsift', keys: ['webhooks/oauth-wh', 'webhooks/snooze-wh', 'webhooks/sendlater-wh'] }).then(function (results) {
        var data = {};
        data.owhurl = results[0].value;
        data.snwhurl = results[1].value;
        data.slwhurl = results[2].value;
        if (state.type === 'email-thread') {
          this$1._tid = state.params.tid;
          data.tid = this$1._tid;
          data.detail = state.params.detail;
          this$1._isAuthenticated().then(function () {
            resolve(data);
          }).catch(function (error) {
            data.signin = true;
            if (error) {
          ...
          this$1.storage.getAll({ bucket: 'snoozedcal' }).then(function (snoozes) {
            var today = moment$1().format('YYYY-MM-DD');
            data.snoozedcal = [];
            snoozes.forEach(function (b) {
              if (b.key >= today) {
                data.snoozedcal.push(JSON.parse(b.value));
              }
            });
            this$1.storage.getAll({ bucket: 'sendlatercal' }).then(function (sendlaters) {
              data.sendlatercal = [];
              sendlaters.forEach(function (s) {
                if (s.key >= today) {
  ...
  JITMailController.prototype.onStorageUpdate = function onStorageUpdate (updated) {
    var this$1 = this;

    updated.forEach(function (u) {
      if (u === '_email.tid') {
        this$1.storage.get({ bucket: '_email.tid', keys: [this$1._tid] }).then(function (result) {
          if (result[0].value) {
            var tinfo = JSON.parse(result[0].value);
            this$1.publish('detail', tinfo.detail);
          }
        });
      }
      else if (u === 'state') {
        this$1._isAuthenticated().then(function () {
          this$1.publish('auth', true);
        }).catch(function (error) {
          this$1.publish('auth', false);
        });
      }
      else if (u === '_email.id' && this$1._did) {
        this$1.storage.get({ bucket: '_email.id', keys: [this$1._did] }).then(function (result) {
          if (result[0].value) {
            this$1.emailclient.close();
          }
        });
      }
      else if (u === 'snoozedcal' || u === 'sendlatercal') {
        this$1.storage.getAll({ bucket: u }).then(function (responses) {
          var today = moment$1().format('YYYY-MM-DD');
          var ret = {};
          ret[u] = [];
          ...
          this$1.publish('calendarupdated', ret);
        });
      }
    });
  };

  JITMailController.prototype._isAuthenticated = function _isAuthenticated () {
    var this$1 = this;

    return new Promise(function (resolve, reject) {
      this$1.storage.get({ bucket: 'state', keys: ['auth'] }).then(function (result) {
        if (result[0].value) {
          var auth = JSON.parse(result[0].value);
          if (auth.valid === true) {
            resolve();
          }
          else {
  ...
  JITMailController.prototype._onGoto = function _onGoto (event) {
    var params;
    switch (event) {
      case 'goto-snoozed':
  ...
  JITMailController.prototype._onIdChanged = function _onIdChanged (event) {
    this._did = event.id;
    // Notify view
    this.publish('idChanged', this._did);
  };

  JITMailController.prototype._onRecipientsChanged = function _onRecipientsChanged (event) {
    // TODO: Should make sure that the draft has at least one recipient for it to be valid
  };

  return JITMailController;
}(SiftController));
