...
function chart(id) {
  ...
  function fullCalendar(dataByDate){
    ...
    const tM = starting.indexOf('utc') > -1 ? utcMonth : month;
    const tD = starting.indexOf('utc') > -1 ? utcDay : day;
  ...
  function hourCalendar(data, inset){
    ...
    var dhMatrix = sequence(24).map(h =>
        sequence(7).map(wd =>{
...
const DEFAULT_INSET$1 = 0;   // scale space
...
function polars(id) {
  ...
  function _impl(context) {
    ...
    selection.each(function() {
      let node = select(this);  
      let sh = height || Math.round(width * DEFAULT_ASPECT$1);
