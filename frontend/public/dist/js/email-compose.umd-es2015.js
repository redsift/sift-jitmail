 ...
 * Copyright (c) 2016 Redsift Limited
 */
var EmailComposeView = (function (SiftView) {
  ...
  if ( SiftView ) EmailComposeView.__proto__ = SiftView;
  EmailComposeView.prototype = Object.create( SiftView && SiftView.prototype );
  EmailComposeView.prototype.constructor = EmailComposeView;

  EmailComposeView.prototype.onLoad = function onLoad () {
    // Initialise time selector
    this._timeSelector.setup();
  };

  EmailComposeView.prototype.onIdChanged = function onIdChanged (id) {
    this._data.draft.id = id;
  };

  EmailComposeView.prototype.onAuth = function onAuth (loggedIn) {
    this._data.signin = !loggedIn;
    this._showView(this._selectView(this._data));
  };

  // TODO: link to docs
  EmailComposeView.prototype.presentView = function presentView (value) {
    var sv = 'loader';
    if (value.data) {
  ...
  // TODO: link to docs
  EmailComposeView.prototype.willPresentView = function willPresentView (value) { };

  EmailComposeView.prototype._onLogin = function _onLogin (state) {
    var sv = 'loader';
    if(state === 'fail') {
      sv = 'signin';
    }
  ...
  EmailComposeView.prototype._selectView = function _selectView () {
    // If user signed out
  ...
  EmailComposeView.prototype._showView = function _showView (id) {
  ...
  EmailComposeView.prototype._setupSendButton = function _setupSendButton () {
    this._sendButton = document.getElementById('send-button');
    this._sendButton.addEventListener('click', this._onSendLater.bind(this));
  };

  EmailComposeView.prototype._onSendLater = function _onSendLater () {
    if (this._data.draft.id) {
      this._showView('loader');
      var wh = new Webhook(this._data.slwhurl);
